// Nityam Products Converter - For scraper output
import fs from 'fs';
...
function convertScrapedProducts() {
  try {
    ...
    const scrapedDataPath = path.join(scriptsDir, 'nityam_all_products_raw.json');
    
    if (!fs.existsSync(scrapedDataPath)) {
      console.log('Scraped data file not found. Please run the scraper first.');
      return;
    }
    
    console.log(`Found scraped data file: ${scrapedDataPath}`);
    
    // Read the scraped data
    const rawData = fs.readFileSync(scrapedDataPath, 'utf8');
    const scrapedProducts = JSON.parse(rawData);
    
    console.log(`Read ${scrapedProducts.length} products from scraped data`);
    ...
    // Generate report on product categories
    const categories = {};
    ...
    // Check if we have pickles
    const pickles = validProducts.filter(p => p.category === 'Pickles');
    console.log(`\nFound ${pickles.length} pickle products:`);
    pickles.forEach(p => {
      console.log(`- ${p.name} (${p.variants.map(v => v.size).join(', ')})`);
    });
